...
export default function Workshop() {
  ...
  const loadData = async () => {
    try {
      setDataLoading(true);
      const response = await fetch('http://localhost:8091/api/vehicles/workshop/list', {
        headers: {
      ...
      console.error('Failed to load workshop vehicles:', error);
    } finally {
  ...
  useEffect(() => {
    const filtered = vehicles.filter(v => {
      ...
      return (
        ...
        v.workshop_name?.toLowerCase().includes(search) ||
  ...
  return (
    ...
    >
      ...
      <TableView vehicles={filteredVehicles} onRefresh={loadData} context="workshop" />
    </Layout>
  );
}
